  ...
  *)
  ...
  remote_head=$(
    if (( ${+commands[curl]} )); then
      curl -fsSLk -H 'Accept: application/vnd.github.v3.sha' $api_url 2>/dev/null
    elif (( ${+commands[wget]} )); then
      wget -O- --no-check-certificate --header='Accept: application/vnd.github.v3.sha' $api_url 2>/dev/null
